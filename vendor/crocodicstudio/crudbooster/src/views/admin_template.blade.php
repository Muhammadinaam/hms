...
<div id='app' class="wrapper">    
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <h1>
            <i class="{{$header_info['page_icon']}}"></i>  {{$header_info['page_title']}} &nbsp;&nbsp;
          </h1>

            @if(isset($header_info['breadcrumb_items']))
            <ol class="breadcrumb">
              <li><a href="{{CRUDBooster::adminPath()}}"><i class="fa fa-dashboard"></i> {{ trans('crudbooster.home') }}</a></li>
              @foreach($header_info['breadcrumb_items'] as $breadcrumb_item)
                @if(!$loop->last)
                <li>
                  <a href="{{$breadcrumb_item['url']}}"> 
                  <i class="{{$breadcrumb_item['icon']}}"></i> 
                  {{breadcrumb_item['name']}}
                </li>
                @else
                <li class="active"> <i class="{{$breadcrumb_item['icon']}}"></i> {{$breadcrumb_item['name']}}</li>
                @endif
              @endforeach
            </ol>
            @endif
