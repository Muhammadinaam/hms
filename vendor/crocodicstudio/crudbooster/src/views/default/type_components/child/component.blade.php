						...
						<div class="panel-body child-form-area">
							...
							<div class='form-group'>
								...
								<div class="col-sm-10">
								    ...
								    <script type="text/javascript">
								    	var url_{{$name_column}} = "{{CRUDBooster::mainpath('modal-data')}}?table={{$col['datamodal_table']}}&columns=id,{{$col['datamodal_columns']}}&joins={{ isset($col['datamodal_joins']) ? $col['datamodal_joins'] : '' }}&name_column={{$name_column}}&where={{urlencode($col['datamodal_where'])}}&select_to={{ urlencode($col['datamodal_select_to']) }}&columns_name_alias={{urlencode($col['datamodal_columns_alias'])}}";
									...
									@elseif($col['type']=='date')
										...
										<div class="input-group">  								
											...
											<input id='{{$name_column}}' name='{{$col["name"]}}' class='form-control input_date {{$col['required']?"required":""}}' readonly />
										</div>
							...
							<script type="text/javascript">
								...
								$(document).on('blur', '#panel-form-{{$name}} [type="number"]', function(){
									var val = $(this).val();
									var min = $(this).attr('min');
									var max = $(this).attr('max');

									if( $.isNumeric(val) == false || val < min )
									{
										$(this).val( min );
									}
						...
						<?php 
							...
							foreach($form['columns'] as $i=>$c) {
								...
								if($c['type'] == 'datamodal') {
									...
									if( strpos($datamodal_title, '.') !== false )
									{
										$data_child->addselect($datamodal_title.' as '. str_replace('.', '_', $datamodal_title) );
									}
									else
									{
							...
							@foreach($form['columns'] as $col)
								...
								<?php 
									if($col['type'] == 'select') {
										...
										echo "<span class='td-label'>";
										$column_name = ( strpos($datamodal_title, '.') !== false ) ? str_replace('.', '_', $datamodal_title) : $datamodal_table.'_'.$datamodal_title;
										echo $d->{$column_name};
										echo "</span>";
										echo "<input type='hidden' name='".$name."-".$col['name']."[]' value='".$d->{$col['name']}."'/>";
									}elseif ($col['type']=='upload') {
