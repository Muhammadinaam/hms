        ...
        <div class='main-menu'>
            ...
            <ul class="sidebar-menu">
                ...
                <?php $dashboard = CRUDBooster::sidebarDashboard();
                    ...
                    CRUDBooster::getCurrentMenuId();
                    CRUDBooster::getCurrentDashboardId();
                ...
                @foreach(CRUDBooster::sidebarMenu() as $menu)
                    ...
                    <li data-id='{{$menu->id}}' class='{{(count($menu->children))?"treeview":""}} {{ $active }}'><a href='{{ ($menu->is_broken)?"javascript:alert('".trans('crudbooster.controller_route_404')."')":$menu->url."?m=".$menu->id }}' class='{{($menu->color)?"text-".$menu->color:""}}'><i class='{{$menu->icon}} {{($menu->color)?"text-".$menu->color:""}}'></i> <span>{{$menu->name}}</span>
                        ...
                        @if(count($menu->children))
                            <ul class="treeview-menu">
                                @foreach($menu->children as $child)
                                    ...
                                    if( (CRUDBooster::getCurrentMenuId()==$child->id) && (CRUDBooster::getCurrentDashboardId()!=$child->id) )
                                    {
                                        $active = 'active';
                                    }
                                    @endphp

                                    <li data-id='{{$child->id}}' class='{{ $active }}'><a href='{{ ($child->is_broken)?"javascript:alert('".trans('crudbooster.controller_route_404')."')":$child->url."?m=".$child->id }}'><i class='{{$child->icon}}'></i> <span>{{$child->name}}</span></a></li>
